defmodule CercleApi.APIV2.CardControllerTest do
  ...
  test "POST/2 create card", state do
    board_channel = "board:#{state[:board].id}"
    CercleApi.Endpoint.subscribe(board_channel)
    response = state[:conn]
    |> post(
    ...
    |> Repo.one
     ...
     assert_receive %Phoenix.Socket.Broadcast{
       topic: board_channel, event: "timeline_event:created", payload: _
     }
    CercleApi.Endpoint.unsubscribe(board_channel)
